...
function logStoreError(scope, error) {
  console.error(`[${scope}] Redis error:`, error);
}
...
export async function getChannels() {
  try {
    ...
    logStoreError("getChannels", error);
    return [];
  }
}
...
export async function getState() {
  try {
    return normalizeState(await redis.get(STATE_KEY));
  } catch (error) {
    logStoreError("getState", error);
    return { ...DEFAULT_STATE };
  }
}
...
export async function getCronStatus() {
  try {
    ...
    logStoreError("getCronStatus", error);
    return null;
  }
}
