...
function isMoviePost(post) {
  const classes = Array.isArray(post?.class_list) ? post.class_list : [];
  const hasTvCategory = classes.some((item) => String(item).startsWith("category-tv-"));
  const title = decodeHtmlEntities(post?.title?.rendered || "");
  const looksLikeSeries = /\bseason\b|\beps?\b|\bcomplete\b/i.test(title);
  return !hasTvCategory && !looksLikeSeries;
}
...
export async function searchMovies(query) {
  ...
  return posts.filter(isMoviePost).map(parseMoviePost);
}
...
export async function latestMovies() {
  ...
  return posts.filter(isMoviePost).slice(0, 10).map(parseMoviePost);
}

export async function getMovieById(id) {
  const post = await fetchJson(`/wp-json/wp/v2/posts/${id}`);
  if (!isMoviePost(post)) {
    throw new Error("Post yang dipilih bukan film");
  }
